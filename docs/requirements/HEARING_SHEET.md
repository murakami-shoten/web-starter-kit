...
| （作成する場合）提供方法：ユーザー提供（Figma等）/ AIが静的HTMLモックで作成 | TBD | Should |

※ユーザーがFigma等で作成済みのワイヤーフレームがあればそれを基に実装する。ユーザー提供が無い場合、AIエージェントがリポジトリ内に静的HTMLモックとして作成・配置する。
※AIエージェントがワイヤーフレームを作成する場合は、`DESIGN_RULES.md` のUI/UX設計原則（ISO 9241-110 / Nielsen ヒューリスティクス / レイアウト・レスポンシブ・導線設計原則）に準拠すること。
