...
- npm / node / npx / lint / test / build などのコマンドは**必ず Docker Compose 経由**で実行する（例: `docker compose run --rm frontend npm run lint`）。ホストで npm / npx を直接実行する前提を置かない。
