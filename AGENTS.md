...
- npm / node / lint / test / build などのコマンドは**必ず Docker Compose 経由**で実行する（例: `docker compose run --rm frontend npm run lint`）。ホストで npm を直接実行する前提を置かない。
...
- APIキーや秘密情報をヒアリングで受領した場合は、その場で内容を `.env.user`（Git管理外）に記録し、シートやIssueには「取得済/未取得」と保管場所のみ記載する。未取得の場合はTBDのまま残し、実装開始前に再確認する。
